...
class PVAdmin
{
    ...
    {
        $this->modx =& $modx;
        $corePath = MODX_CORE_PATH . 'components/pvadmin/';
        // $assetsUrl = MODX_ASSETS_URL . 'components/pvadmin/';
        ...
        $this->modx->addPackage('pvadmin', $this->config['modelPath']);
        // $this->gtsShop = $modx->getService("gtsShop","gtsShop",
        //     MODX_CORE_PATH."components/gtsshop/model/",[]);
        // //$this->modx->lexicon->load('pvadmin:default');
    ...
    /**
     * Регистрация триггеров gtsAPI
     */
    public function regTriggers() {
        return [
            'modUser' => [
                'gtsapifunc' => 'handleUserTrigger'
            ]
        ];
    }
    
    /**
     * Триггер для обработки операций с пользователями
     */
    public function handleUserTrigger($params) {
        $type = $params['type'];
        $method = $params['method'];
        
        // Обработка пароля перед созданием или обновлением пользователя
        if ($type === 'before' && in_array($method, ['create', 'update'])) {
            $data = &$params['object_new'];

            // Если пароль пустой при обновлении, удаляем его из данных
            if ($method === 'update') {
                unset($data['password']);
            }
